<template>
  <dl class="s-item">
    ...
    <dd>
      <h3>
        <router-link :to="{name: 'productionDetail'}">{{meta.title}}</router-link>
      </h3>
      <p>
        <el-rate
          v-model="meta.score"
      ...
      <p>
        <span class="s-item-tab">
          <span v-for="(item,i ) in meta.tab" :key="i">{{item + (i < item.length - 1 ? '&nbsp | &nbsp' : '')}}</span>
        </span>
        <span class="s-item-addr">{{meta.address}}</span>
        <span><a href="#" class="el-icon-position" style="text-decoration: none">去这里</a></span>
      </p>
      <p>
